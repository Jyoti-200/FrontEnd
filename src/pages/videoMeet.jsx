...
import { Cast, Stream } from "@mui/icons-material";
...
export default function VideoMeetComponent(){
    ...
    let [newMessages, setNewMesaages] = useState(0);
        ...
        try{
           ...
           for(let id in connection){
            ...
            connection[id].createOffer().then((description)=> {
                ...
                .then(()=>{
                    socketIdRef.current.emit("signal", id, JSON.stringify({"sdp": connection[id].loacalDescription}))
                })
                .catch(e => console.log(e))
            })
           }
           ...
           stream.getTracks().forEach(track => track.onended = () => {
            ...
            for(let id in connection){
                ...
                connection[id].createOffer().then((description)=> {
                    connection[id].setLocalDescription(description)
                    .then(() => {
                        socketRef.current.emit("signal", id, JSON.stringify({"sdp":connection[id].loacalDescription}))
                    }).catch(e => console.log(e));
                })
            }
                ...
                if(signal.sdp.type === "offer"){
                    connection[fromId].createAnswer().then((description)=>{
                        connection[fromId].setLocalDescription(description).then(()=>{
                            socketIdRef.current.emit("signal", fromId, JSON.stringify({"sdp": connection[fromId].loacalDescription}));
                        }).catch(e => console.log(e));
                    }).catch(e => console.log(e));
                }
            }).catch(e => console.log(e));
                         ...
                         try{
                          ...
                          connection[id2].createOffer().then((description)=>{
                              connection[id2].setLocalDescription(description)
                               .then(()=>{ 
                                socketRef.current.emit("signal", id2, JSON.stringify({"sdp":connection[id2].setLocalDescription}))
